  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Total Gaming Match Center" },
      {
        ...
        content: "Live Total Gaming tournament standings, match history, and player statistics.",
      },
...
function Dashboard() {
  ...
  const activeEventName =
    selectedEvent && eventNames.includes(selectedEvent) ? selectedEvent : (eventNames[0] ?? null);
  ...
  const standings = useMemo(() => {
    const rows = new Map<
      string,
      { id: string | null; name: string; points: number; kills: number }
    >();
  ...
  const selectedMatch =
    tournamentData.matches.find((match) => match.id === selectedMatchId) ?? null;
  ...
  function selectEvent(name: string) {
    ...
    const first =
      next.find((tournament) => tournament.isCurrent || tournament.status === "LIVE") ?? next[0];
  ...
  return (
    <div className="tg-dashboard-v2">
      ...
      <main id="top">
        <section className="tg-current-hero tg-page-width">
          <div className="tg-hero-content">
            <p className="tg-overline">
              <span className="tg-pulse" /> CURRENT TOURNAMENT
            </p>
            <p className="tg-circuit-label">
              {mode === "official" ? "OFFICIAL CIRCUIT" : "SCRIMS CIRCUIT"}
            </p>
            <h1>
              {loadingEvents
                ? "Loading tournament"
                : (selectedTournament?.name ?? "No active tournament")}
            </h1>
            ...
            <div className="tg-hero-facts">
              ...
              <Fact
                label="MATCHES"
                value={selectedTournament ? String(selectedTournament.matches) : "—"}
              />
              <Fact
                label="TEAMS"
                value={selectedTournament ? String(selectedTournament.teams) : "—"}
              />
            </div>
          </div>
        ...
        <section className="tg-control-band tg-page-width" id="events">
          <div className="tg-control-heading">
            <div>
              <p className="tg-overline">
                <span /> CIRCUIT SELECTOR
              </p>
              <h2>Choose your view</h2>
            </div>
            <span className="tg-record-count">
              {eventNames.length} {eventNames.length === 1 ? "EVENT" : "EVENTS"}
            </span>
          </div>
          ...
          <div className="tg-event-list">
            {loadingEvents ? (
              <LoadingLine label="Loading published events" />
            ) : eventNames.length === 0 ? (
              <EmptyState label={`No ${mode} events have been published yet.`} />
            ) : (
              eventNames.map((name) => {
                const eventStages = tournaments.filter((tournament) => tournament.name === name);
                const live = eventStages.some(
                  (tournament) => tournament.isCurrent || tournament.status === "LIVE",
                );
                return (
                  <button
                    type="button"
                    className={`tg-event-button ${activeEventName === name ? "is-active" : ""}`}
                    key={name}
                    onClick={() => selectEvent(name)}
                  >
                    <span className="tg-event-icon">
                      <Layers3 size={18} />
                    </span>
                    <span className="tg-event-copy">
                      <strong>{name}</strong>
                      <small>
                        {eventStages.length} {eventStages.length === 1 ? "stage" : "stages"}
                      </small>
                    </span>
          ...
          {eventTournaments.length > 0 && (
            <div className="tg-stage-strip">
              <div className="tg-stage-strip-label">
                <CalendarDays size={16} /> STAGES / DAYS
              </div>
              <div className="tg-stage-options">
                {eventTournaments.map((tournament) => (
                  <button
                    type="button"
                    key={tournament.id}
                    className={`tg-stage-button ${selectedTournament?.id === tournament.id ? "is-active" : ""}`}
        ...
        <section className="tg-stage-summary tg-page-width" aria-labelledby="summary-title">
          ...
          {loadingData ? (
            <LoadingLine label="Loading stage performance" />
          ) : (
            <div className="tg-summary-grid">
              <MetricCard
                icon={Trophy}
                label="TOTAL POINTS"
                value={totalPoints ? totalPoints.toString() : "—"}
                accent
                detail={
                  teamResults.length
                    ? `${teamResults.length} matches scored`
              ...
              <MetricCard
                ...
                detail={
                  rank
                    ? selectedTournament?.description || "Live standings"
                    : "Ranking appears after results"
                }
              />
              <MetricCard
                icon={Crosshair}
                label="KILLS"
                value={teamResults.length ? totalKills.toString() : "—"}
                detail={
                  teamResults.length
                    ? `${averagePoints} points / match`
                    : "No team results published"
                }
              />
              <MetricCard
                ...
                value={teamResults.length ? placementPoints.toString() : "—"}
                detail={
                  teamResults.length ? "Calculated from team score" : "No team results published"
                }
              />
            </div>
          )}
        ...
        <section className="tg-history tg-page-width" id="history" aria-labelledby="history-title">
          ...
          {loadingData ? (
            <LoadingLine label="Loading matches" />
          ) : tournamentData.matches.length === 0 ? (
            <EmptyState label="No matches have been published for this stage yet." />
          ) : (
            <div className="tg-match-list">
              <div className="tg-match-list-head">
                <span>MATCH</span>
                <span>MAP</span>
                <span>KILLS</span>
                <span>POSITION</span>
              ...
              {tournamentData.matches.map((match) => {
                ...
                return (
                  <MatchRow
                    ...
                    match={match}
                    result={result}
                    onStats={() => setSelectedMatchId(match.id)}
                  />
                );
              })}
            </div>
        ...
        <section
          className="tg-fraggers tg-page-width"
          id="fraggers"
          aria-labelledby="fraggers-title"
        >
          <div className="tg-section-topline">
            <div>
              <p className="tg-overline">
                <span /> PLAYER PERFORMANCE
              </p>
          ...
          {loadingData ? (
            <LoadingLine label="Loading player performance" />
          ) : topFraggers.length === 0 ? (
            <EmptyState label="Player statistics have not been published for this stage yet." />
          ) : (
            <div className="tg-fragger-list">
              {topFraggers.map((entry, index) => (
                <FraggerRow key={entry.player.id} entry={entry} index={index} />
              ))}
            </div>
          )}
        </section>

        <footer className="tg-footer-v2 tg-page-width">
          <span>TOTAL GAMING HUB</span>
          <small>Official results · Scrims · Player stats</small>
        </footer>
      </main>
      ...
      {selectedMatch && (
        <StatsModal
          match={selectedMatch}
          stats={tournamentData.playerMatchStats.filter(
...
function MetricCard({
  ...
  return (
    <article className={`tg-metric-card ${accent ? "is-accent" : ""}`}>
      <span className="tg-metric-icon">
        <Icon size={18} />
      </span>
      <span className="tg-metric-label">{label}</span>
      <strong>{value}</strong>
      <small>{detail}</small>
    </article>
  );
...
function MatchRow({
  ...
  return (
    <div className="tg-match-row">
      ...
      <span className="tg-map-name">{match.map || "Map pending"}</span>
      <strong>{result ? result.kills : "—"}</strong>
      <strong>{result?.position ? `#${result.position}` : "—"}</strong>
      <strong className="tg-points-value">{result ? result.points : "—"}</strong>
      <button type="button" className="tg-stats-button" onClick={onStats}>
        Full stats <ArrowUpRight size={15} />
      </button>
    </div>
  );
}
...
function FraggerRow({
  ...
  return (
    <div className="tg-fragger-row">
      <span className="tg-rank-number">{String(index + 1).padStart(2, "0")}</span>
      <Avatar player={entry.player} />
      <div className="tg-fragger-name">
        <strong>{entry.player.name}</strong>
        <small>
          {entry.player.role} · {entry.matches} matches
        </small>
      </div>
      <div className="tg-fragger-stat">
        <span>KILLS</span>
        <strong>{entry.kills}</strong>
      </div>
      <div className="tg-fragger-stat tg-fragger-damage">
        <span>DAMAGE</span>
...
function Avatar({ player }: { player: Player }) {
  const initials = player.name
    ...
    .toUpperCase();
  return (
    <span className="tg-player-avatar">
      {player.avatarUrl ? <img src={player.avatarUrl} alt={`${player.name} portrait`} /> : initials}
    </span>
  );
}
    ...
    >
      ...
      >
        <div className="tg-modal-head">
          <div>
            <p className="tg-overline">
              <span /> MATCH #{String(match.number).padStart(2, "0")}
            </p>
            <h2 id="stats-modal-title">{match.map || "Match stats"}</h2>
          </div>
          <button
            type="button"
            className="tg-close-button"
        ...
        <div className="tg-player-stat-list">
          {stats.length === 0 ? (
            <EmptyState label="Individual player stats have not been published for this match." />
          ) : (
            stats.map((stat) => {
              const player = playerMap.get(stat.playerId);
              if (!player) return null;
              return (
                <div className="tg-player-stat" key={stat.id}>
                  <Avatar player={player} />
                  <div className="tg-player-stat-name">
